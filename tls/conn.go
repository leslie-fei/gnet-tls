...
var ErrNotEnough = errors.New("data not enough")
...
func (c *Conn) SyscallConn() (syscall.RawConn, error) {
	return c.conn.(*net.TCPConn).SyscallConn()
}

func (c *Conn) SetReadBuffer(bytes int) error {
	return c.conn.(*net.TCPConn).SetReadBuffer(bytes)
}

func (c *Conn) SetWriteBuffer(bytes int) error {
	return c.conn.(*net.TCPConn).SetWriteBuffer(bytes)
}
...
func (c *Conn) SetKeepAlive(keepalive bool) error {
	return c.conn.(*net.TCPConn).SetKeepAlive(keepalive)
}
...
func (c *Conn) readRecordOrCCS(expectChangeCipherSpec bool) error {
	...
	if err := c.readFromUntil(c.conn, n); err != nil {
		if e, ok := err.(net.Error); !ok || !e.Temporary() {
			if !errors.Is(err, ErrNotEnough) {
				c.in.setErrorLocked(err)
			}
...
func (c *Conn) HandshakeCompleted() bool {
...
func (c *Conn) CanRead() error {
	...
	if r.InboundBuffered() < recordHeaderLen {
		return ErrNotEnough
	}
	...
	n := int(hdr[3])<<8 | int(hdr[4])
	total := n + recordHeaderLen
	if r.InboundBuffered() < total {
		return ErrNotEnough
	}
	return nil
}
